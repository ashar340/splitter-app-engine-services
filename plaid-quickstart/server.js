...
const nhostEndpoint = 'https://fytcssmgfmprizwxxnyb.nhost.run';
...
app.get('/', function (request, response, next) {
  response.json({
    message: 'Hello from Plaid\'s Node.js Quickstart!'
  })
});
...
app.post('/api/set_access_token', function (request, response, next) {
  ...
  let nhostAuthToken = request.body.authToken;
  ...
  Promise.resolve()
    .then(async function () {
      ...
      const nhostClient = new GraphQLClient(nhostEndpoint, {
        headers: {
          'Authorization': `Bearer ${nhostAuthToken}`,
        }
      });

      const query = gql`
      mutation UpdatePlaidToken($id: uuid!, $plaid_access_token: String) {
  updateUser(pk_columns: {id: $id}, _set: {plaid_access_token: $plaid_access_token}) {
    displayName
    plaid_access_token
  }
}
`;
      var res = nhostClient.request(query, {
